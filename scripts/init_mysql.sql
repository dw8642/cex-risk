...
CREATE TABLE IF NOT EXISTS `risk_rules` (
    ...
    `description` TEXT,
    `category`    ENUM('permission','behavior','system','exposure','liquidation') NOT NULL,
...
-- 风控规则定义（10 条，演示版只启用前 2 条）
INSERT INTO `risk_rules` (`id`, `rule_code`, `name`, `description`, `category`, `priority`, `enabled`, `config`) VALUES
('r001', 'P-001', '提币权限异常开启',   '检测 API Key 提币权限状态与预期配置不符',                   'permission',  'P0', 1, '{"check_interval":"30s"}'),
('r002', 'P-002', '非授权交易范围开放',  '检测账户在非授权交易对上发生了交易活动',                    'permission',  'P0', 1, '{"check_interval":"10s"}'),
...
('r009', 'E-001', '净Delta过高',         '账户或项目级净 Delta 暴露超过风险预算',                    'exposure',    'P0', 0, '{"max_delta_usd": 100000}'),
('r010', 'L-001', '维持保证金率过低',    '账户维持保证金率低于安全阈值，存在强平风险',               'liquidation', 'P0', 0, '{"min_margin_ratio": 0.1}')
...
-- 演示账户（需要填入真实的币安账户标识）
INSERT INTO `accounts` (`id`, `project_id`, `partner_id`, `exchange_id`, `market_type`, `label`, `status`) VALUES
('acc-demo-001', 'proj-demo-001', 'partner-demo-001', 'binance', 'futures', '演示合约账户-01', 'active')
